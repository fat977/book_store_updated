...
@section('title',__('dashboard/sidebar.address.regions'))
@section('css')
<!-- DataTables -->
...
<div class="content-wrapper">
    <div class="row p-4">
        <div class="col-12">
            <a href="{{ route('admin.regions.create') }}" class="btn btn-dark">{{__('dashboard/sidebar.address.add_region')}}</a>
            <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{$region->id}}</td>
                        <td>{{$region->name}}</td>
                        <td> <span class="badge badge-{{$region->status == 0 ? 'danger' : 'success'}} text-center">{{$region->status == 0 ? __('dashboard/sidebar.commons.not_active') : __('dashboard/sidebar.commons.active')}}</span></td>
                        <td>{{$region->city->name}}</td>
                        <td>
                            <a href="{{route('admin.regions.edit',$region->id)}}" class="btn btn-warning">{{__('dashboard/sidebar.commons.edit')}}</a>
                            <button type="button" class="btn btn-danger" data-toggle="modal"
                                data-target="#confirm-delete-{{ $region->id }}">
                                {{__('dashboard/sidebar.commons.delete')}}
                            </button>
                            <div class="modal fade" id="confirm-delete-{{ $region->id }}">
